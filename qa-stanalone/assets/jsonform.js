...
(function (serverside, global, $, _, JSON) {
  ...
  var initializeTabs = function (tabs) {
    ...
    var optionSelected = function (e) {
      ...
      $target = $(this)
        .parents('.tabbable')
        .eq(0)
        .find('> .tab-content > [data-idx=' + targetIdx + ']');
  ...
  jsonform.elementTypes = {
    ...
    wysihtml5: {
      template:
        '<textarea id="<%= id %>" name="<%= node.name %>" style="height:<%= elt.height || "300px" %>;width:<%= elt.width || "100%" %>;"' +
        '<%= (fieldHtmlClass ? "class=\'" + fieldHtmlClass + "\' " : "") %>' +
        '<%= (node.disabled? " disabled" : "")%>' +
        '<%= (node.readOnly ? " readonly=\'readonly\'" : "") %>' +
        '<%= (node.schemaElement && node.schemaElement.maxLength ? " maxlength=\'" + node.schemaElement.maxLength + "\'" : "") %>' +
    ...
    ace: {
      template:
        '<div id="<%= id %>" style="position:relative;height:<%= elt.height || "300px" %>;"><div id="<%= id %>__ace" style="width:<%= elt.width || "100%" %>;height:<%= elt.height || "300px" %>;"></div><input type="hidden" name="<%= node.name %>" id="<%= id %>__hidden" value="<%= escape(value) %>"/></div>',
      fieldtemplate: true,
      inputfield: true,
      onInsert: function (evt, node) {
        var setup = function () {
          ...
          var editor = ace.edit(
            $(node.el)
              .find('#' + escapeSelector(node.id) + '__ace')
              .get(0)
          );
    ...
    checkbox: {
      template:
        '<div class="checkbox"><label><input type="checkbox" id="<%= id %>" ' +
        '<%= (fieldHtmlClass ? " class=\'" + fieldHtmlClass + "\'": "") %>' +
    ...
    'file-hosted-public': {
      template:
        '<span><% if (value && (value.type||value.url)) { %>' +
        fileDisplayTemplate +
        '<% } %><input class="input-file" id="_transloadit_<%= id %>" type="file" name="<%= transloaditname %>" /><input data-transloadit-name="_transloadit_<%= transloaditname %>" type="hidden" id="<%= id %>" name="<%= node.name %>" value=\'<%= escape(JSON.stringify(node.value)) %>\' /></span>',
      fieldtemplate: true,
      inputfield: true,
      getElement: function (el) {
        return $(el).parent().get(0);
    ...
    'file-transloadit': {
      template:
        '<span><% if (value && (value.type||value.url)) { %>' +
        fileDisplayTemplate +
        '<% } %><input class="input-file" id="_transloadit_<%= id %>" type="file" name="_transloadit_<%= node.name %>" /><input type="hidden" id="<%= id %>" name="<%= node.name %>" value=\'<%= escape(JSON.stringify(node.value)) %>\' /></span>',
      fieldtemplate: true,
      inputfield: true,
      getElement: function (el) {
        return $(el).parent().get(0);
      ...
      onChange: function (evt, elt) {
        ...
        formElt.transloadit({
          ...
          onSuccess: function (assembly) {
            ...
            _.each(results, function (result) {
              // Save the assembly result in the right hidden input field
              var input = formElt.find('input[name="' + result.field.replace(/^_transloadit_/, '') + '"]');
              var nonEmptyKeys = _.filter(_.keys(result.meta), function (key) {
                return !!isSet(result.meta[key]);
              });
              result.meta = _.pick(result.meta, nonEmptyKeys);
    ...
    imageselect: {
      ...
      onInsert: function (evt, node) {
        $(node.el).on('click', '.dropdown-menu a', function (evt) {
          evt.preventDefault();
          evt.stopPropagation();
          var img = evt.target.nodeName.toLowerCase() === 'img' ? $(evt.target) : $(evt.target).find('img');
    ...
    iconselect: {
      ...
      onInsert: function (evt, node) {
        $(node.el).on('click', '.dropdown-menu a', function (evt) {
          evt.preventDefault();
          evt.stopPropagation();
          var i = evt.target.nodeName.toLowerCase() === 'i' ? $(evt.target) : $(evt.target).find('i');
    ...
    radios: {
      template:
        '<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %><div class="radio"><label><input<%= (fieldHtmlClass ? " class=\'" + fieldHtmlClass + "\'": "") %> type="radio" <% if (((key instanceof Object) && (value === key.value)) || (value === key)) { %> checked="checked" <% } %> name="<%= node.name %>" value="<%= (key instanceof Object ? key.value : key) %>"' +
        '<%= (node.disabled? " disabled" : "")%>' +
        '<%= (node.schemaElement && node.schemaElement.required ? " required=\'required\'" : "") %>' +
    ...
    array: {
      ...
      onInsert: function (evt, node) {
        ...
        $('> span > a._jsonform-array-addmore', $nodeid).click(function (evt) {
          ...
          if (boundaries.maxItems >= 0) {
            if (node.children.length > boundaries.maxItems - 2) {
              $nodeid.find('> span > a._jsonform-array-addmore').addClass('disabled');
            }
          ...
          if (boundaries.minItems <= 0 || (boundaries.minItems > 0 && node.children.length > boundaries.minItems - 1)) {
            $nodeid.find('> span > a._jsonform-array-deletelast').removeClass('disabled');
          }
        });
        ...
        if (boundaries.minItems > 0 && node.children.length <= boundaries.minItems) {
          $nodeid.find('> span > a._jsonform-array-deletelast').addClass('disabled');
        }
        ...
        $('> span > a._jsonform-array-deletelast', $nodeid).click(function (evt) {
          ...
          if (boundaries.maxItems >= 0 && idx <= boundaries.maxItems - 1) {
            $nodeid.find('> span > a._jsonform-array-addmore').removeClass('disabled');
          }
        });
    ...
    tabarray: {
      template:
        '<div id="<%= id %>"><div class="tabbable tabs-left">' +
      ...
      onInsert: function (evt, node) {
        ...
        $('> a._jsonform-array-deleteitem', $nodeid).click(function (evt) {
          ...
          node.deleteArrayItem(idx);
          updateTabs();
          if (node.children.length < boundaries.minItems + 1 || node.children.length === 0) {
            $nodeid.find('> a._jsonform-array-deleteitem').addClass('disabled');
          }
        ...
        $('> a._jsonform-array-addmore', $nodeid).click(function (evt) {
          ...
          updateTabs(idx);
          if (boundaries.minItems <= 0 || (boundaries.minItems > 0 && idx > boundaries.minItems - 1)) {
    ...
    fieldset: {
      template:
        '<fieldset class="form-group jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
        '<% if (id) { %> id="<%= id %>"<% } %>' +
        '>' +
    ...
    submit: {
      template:
        '<input type="submit" <% if (id) { %> id="<%= id %>" <% } %> class="btn btn-primary <%= elt.htmlClass?elt.htmlClass:"" %>" value="<%= value || node.title %>"<%= (node.disabled? " disabled" : "")%>/>'
    },
    button: {
      template:
        ' <button type="button" <% if (id) { %> id="<%= id %>" <% } %> class="btn btn-default <%= elt.htmlClass?elt.htmlClass:"" %>"><%= node.title %></button> '
    },
    actions: {
    ...
    selectfieldset: {
      ...
      onBeforeRender: function (data, node) {
        ...
        if (node.options) {
          children = _.map(node.options, function (option, idx) {
            ...
            if (option instanceof Object) {
              ...
              option.value = isSet(option.value) ? option.value : isSet(choices[idx]) ? choices[idx] : idx;
              return option;
            } else {
              return {
                title: option,
                value: isSet(choices[child.childPos]) ? choices[child.childPos] : child.childPos,
                node: child
              };
            }
          });
        ...
        var elt = node.formElement;
        var tabs = '<select class="nav"' + (node.disabled ? ' disabled' : '') + '>';
    ...
    questions: {
      template:
        '<div>' + '<input type="hidden" id="<%= node.id %>" name="<%= node.name %>" value="<%= escape(value) %>" />' + '<%= children %>' + '</div>',
      fieldtemplate: true,
      inputfield: true,
      getElement: function (el) {
        return $(el).parent().get(0);
    ...
    question: {
      template:
        '<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %><label class="<%= (node.formElement.optionsType === "radiobuttons") ? "btn btn-default" : "" %><%= ((key instanceof Object && key.htmlClass) ? " " + key.htmlClass : "") %>"><input type="radio" <% if (node.formElement.optionsType === "radiobuttons") { %> style="position:absolute;left:-9999px;" <% } %>name="<%= node.id %>" value="<%= val %>"<%= (node.disabled? " disabled" : "")%>/><span><%= (key instanceof Object ? key.title : key) %></span></label> <% }); %></div>',
      fieldtemplate: true,
      onInsert: function (evt, node) {
        var activeClass = 'active';
        var elt = node.formElement || {};
  ...
  jsonform.util.getObjKey = function (obj, key, ignoreArrays) {
    ...
    for (var i = 0; i < keyparts.length; i++) {
      ...
      } else if (ignoreArrays && !innerobj[prop] && _.isArray(innerobj) && innerobj[0]) {
  ...
  var getSchemaKey = function (schema, key) {
    var schemaKey = key.replace(/\./g, '.properties.').replace(/\[[0-9]*\]/g, '.items');
    var schemaDef = jsonform.util.getObjKey(schema, schemaKey, true);
    if (schemaDef && schemaDef.$ref) {
      throw new Error('JSONForm does not yet support schemas that use the ' + '$ref keyword. See: https://github.com/joshfire/jsonform/issues/54');
    }
    return schemaDef;
  };
  ...
  formNode.prototype.computeInitialValues = function (values, ignoreDefaultValues) {
    ...
    if (this.formElement) {
      ...
      _.each(
        ['title', 'legend', 'description', 'append', 'prepend', 'inlinetitle', 'helpvalue', 'value', 'disabled', 'placeholder', 'readOnly'],
        function (prop) {
  ...
  formNode.prototype.getFormValues = function (updateArrayPath) {
    ...
    formArray = formArray.concat(
      $(':input[type=checkbox]:not(:disabled):not(:checked)', this.el)
        .map(function () {
  ...
  formNode.prototype.getArrayBoundaries = function () {
    ...
    var getNodeBoundaries = function (node, initialNode) {
      ...
      if (node.key) {
        ...
        schemaKey = getSchemaKey(node.ownerTree.formDesc.schema.properties, arrayKey);
  ...
  formTree.prototype.buildFromLayout = function (formElement, context) {
    ...
    if (formElement.key) {
      ...
      formElement.readOnly = formElement.readOnly || schemaElement.readOnly || formElement.readonly || schemaElement.readonly;

      // Compute the ID of the input field
      if (!formElement.id) {
        formElement.id = escapeSelector(this.formDesc.prefix) + '-elt-' + slugify(formElement.key);
      }
      ...
      if (!formElement.type) {
        ...
        } else if ((schemaElement.type === 'number' || schemaElement.type === 'integer') && !schemaElement['enum']) {
          formElement.type = 'number';
          if (schemaElement.type === 'number') schemaElement.step = 'any';
        } else if ((schemaElement.type === 'string' || schemaElement.type === 'any') && !schemaElement['enum']) {
    ...
    if (schemaElement) {
      ...
      if (!view.inputfield && !view.array && formElement.type !== 'selectfieldset' && schemaElement.type !== 'object') {
        throw new Error(
          'The JSONForm contains an element that links to an ' +
            'element in the JSON schema (key: "' +
            formElement.key +
            '") ' +
            'and that should not based on its type ("' +
            formElement.type +
            '")'
        );
      }
      ...
      if (view.inputfield && formElement.type !== 'selectfieldset') {
        throw new Error(
          'The JSONForm defines an element of type ' +
            '"' +
            formElement.type +
            '" ' +
  ...
  $.fn.jsonFormErrors = function (errors, options) {
    ...
    for (var i = 0; i < errors.length; i++) {
      ...
      $(errormarkerclass + ' .jsonform-errortext', this)
